...
#   deploy/ship.sh user@phev-host           # or pass the target as an argument
#
# The server must already be set up once (see deploy/README.md): docker login ghcr.io,
# the deploy/ folder present with a filled-in .env.
set -euo pipefail

SERVER="${1:-${PHEV_SERVER:?Set PHEV_SERVER=user@host or pass it as the first argument}}"
REMOTE_DIR="${PHEV_REMOTE_DIR:-phev-sweetspot/deploy}"
